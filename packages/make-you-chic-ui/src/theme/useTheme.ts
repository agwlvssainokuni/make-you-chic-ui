...
const noop = (): void => {
  if (import.meta.env?.DEV) {
    console.warn(
      '[make-you-chic-ui] useTheme() setter called outside of <ThemeProvider>. This call was ignored.',
    )
  }
}
...
export function useTheme(): ThemeContextValue {
  ...
  if (ctx === null) {
    if (import.meta.env?.DEV) {
      console.warn(
        '[make-you-chic-ui] useTheme() was called outside of <ThemeProvider>. Returning default (non-reactive) values.',
      )
    }
