...
export function useControllableState<T>({
  ...
  useEffect(() => {
    if (import.meta.env?.DEV && wasControlled.current !== isControlled) {
      console.warn(
        '[make-you-chic-ui] A component switched between controlled and uncontrolled. Decide between using a controlled or uncontrolled value for the lifetime of the component.',
      )
    }
