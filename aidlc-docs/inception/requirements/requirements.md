...
- **FR8 Webフォントのセルフホスティング**: Noto Sans JP / Noto Serif JP を自己ホスティングし、CDNに依存しない(Google FontsのCDN配信URLは使用しない)。React版は`@fontsource/noto-sans-jp` / `@fontsource/noto-serif-jp`(`dependencies`)への依存を通じて実現し、利用側プロジェクトが自身のバンドルに組み込む(Unit 9のレビュー対応で判明: 当初の手動ダウンロード方式は日本語グリフを含まない誤ったファイルであり、かつデザインシステム自身のCSSバンドルに`@font-face`ごと含めるとVite libraryビルドがフォント実体をサイズ上限なくbase64インライン化しCSSが約30MBに肥大化する問題があったため)。Node.js不要のHTML版(FR6)はビルドチェーンを持たないため、引き続きwoff2実体とOFLライセンス全文を`html-demo/assets/fonts/`に直接同梱する
