...
- **Status**: `react/jsx-runtime`のexternal化+dist再ビルドが完了、コミット`b9f5295`として反映済み。消費側`resolve.dedupe`への`react/jsx-runtime`追加要否の質問に回答済み(コード変更なし)。Approve & Continueはユーザーからの明示的な合図があるまで提示しない
