...
- **Status**: `react/jsx-runtime`のexternal化+dist再ビルドが完了、コミット待ち。Approve & Continueはユーザーからの明示的な合図があるまで提示しない
