...
If a consuming application later wants to load-test its own usage of this library (e.g. large-`Table` render performance, many concurrent `Toast`s), that would be a performance test of the _consuming app_, not of this component library, and is out of scope here.
