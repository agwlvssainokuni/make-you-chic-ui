...
- **`npm pack` shipped no code** (2026-08-12): a real `npm pack` dry run (requested by the user specifically to verify the tarball-based integration path documented in `docs/integration-guide.md`) revealed `package.json` had no `files` field and no `.npmignore`, so npm fell back to `.gitignore` — which excludes `dist/`. The packed tarball contained almost the entire repository (280 files, 5.1MB: `src/`, `aidlc-docs/`, `.idea/`, etc.) but not the one directory the package actually needs. Fixed with `"files": ["dist"]`. Re-verified end-to-end with a throwaway consumer project: `npm install <tarball>`, then real component usage, `tsc --noEmit`, `vite build`, and a dev-server `curl` smoke test all passed against the corrected 42.5kB/72-file tarball.
