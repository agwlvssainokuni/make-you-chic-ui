...
複数要素の並び(flex/grid)を伴う画面固有のレイアウトについて、本デザインシステムは汎用レイアウトコンポーネント(`<Stack>`/`<Grid>`等)を意図的に提供しません。理由や具体的な書き方は[`.claude/skills/layout-css/SKILL.md`](../.claude/skills/layout-css/SKILL.md)にAI Coding Agent向けSkillとしてまとめてあります。利用側プロジェクトでAI(Claude Code等)にレイアウト実装を任せる場合は、このディレクトリを利用側プロジェクトの`.claude/skills/`にコピーしてください(Skillはプロジェクトスコープのため、自動的には共有されません)。
